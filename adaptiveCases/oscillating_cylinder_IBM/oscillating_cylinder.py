...
#############################        Domain & Waves      ################################
# --- Domain
if not opts.genMesh and opts.usePUMI:
    ...
    domain.PUMIManager.PUMIAdapter.loadModelAndMesh(b"Reconstructed.dmg", b"4-Proc/.smb")
    modelDict = {'flow':1,'phase':3,'corrections':[4,5]}
    domain.PUMIManager.modelDict = modelDict
    domain.PUMIManager.sizeInputs = [b'interface',b'error_vms']
    domain.PUMIManager.adapt = 1
    domain.PUMIManager.hmax = he#*4.0
    domain.PUMIManager.hmin= he/2.0
    domain.PUMIManager.hphi= he/2.0
    domain.PUMIManager.numAdaptSteps= 10
    domain.PUMIManager.numIterations= 5
    domain.PUMIManager.targetError= 0.1
    domain.PUMIManager.gradingFactor= 1.5
    domain.PUMIManager.logging= 0
...
def forcedMotion(t):
    return np.array([0.5, opts.At[1]*np.sin(2*np.pi/opts.Tt[1]*t)+opts.tank_dim[1]/2,0.,0.0])
#body.setPrescribedMotion(forcedMotion)
...
body.setIBM(True,
            radiusIBM=opts.radius,  # used only when particle are balls
...
m.rans2p.p.coefficients.use_ball_as_particle = 1
...
m.rans2p.p.coefficients.eb_bc_penalty_constant = 100.#/nu_0#Re
